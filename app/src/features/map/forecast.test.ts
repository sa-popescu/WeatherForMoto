...
describe('parseForecast', () => {
  ...
  it('stops after the requested number of hours', () => {
    const data = parseForecast([point([10, 20, 30, 40], [0, 0, 0, 0])], 1, NOW, 2);
  ...
  it('accepts a single object and rejects junk', () => {
    expect(parseForecast(point([10, 20, 30, 40], [0, 0, 0, 0]), 1, NOW)?.times[0]).toBe('2026-09-12T09:00');
    expect(parseForecast(null, 1, NOW)).toBeNull();
    expect(parseForecast([], 1, NOW)).toBeNull();
    expect(parseForecast([{ hourly: { time: [] } }], 1, NOW)).toBeNull();
  });
});
...
describe('colours', () => {
  ...
  it('gets more opaque as the sky fills in', () => {
